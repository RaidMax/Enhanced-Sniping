...
// this is the default damage file we need to make some edits for the knife round
...
{
	// this function is called when a player is killed / victim dies so we want to see if knife damage should be 0 or normal. Round 0 should be normal damage otherwise 0.
...
doFinalKillcam( delay, victim, attacker, attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, deathTimeOffset, psOffsetTime )
{
	...
	foreach ( player in level.players )
	{
		...
		player VisionSetNakedForPlayer( "so_bridge", 0 );
		player setclientdvar( "r_blur", .65 );
...
{	
	...
	if (sMeansOfDeath == "MOD_MELEE") //MOD_MELEE == knife
	{
		if (level.WarmUpRound) // this really means if ( level.knifeRound == true )
		{
			iDamage = 100; // normal player health is 100, so this should inflict 100 damage to player
		}

		if (!level.WarmUpRound) // this really means if (level.knifeRound == false), hence the ! is not so != mean not equal so you could do if ("test" != "test2") it would always be false because the string "test" is not the same as "test2"
		{
			iDamage = 0; // we dont want players knifing on normal rounds do we?
		}
	}
...
_obituary( victim, attacker, sWeapon, sMeansOfDeath )
{
	...
	killwords[0] = " gibbed ";
	killwords[1] = " killed ";
	killwords[2] = " slaughtered ";
	killwords[3] = " destroyed ";
	killwords[4] = " raped ";
	killwords[5] = " fragged ";
	killwords[6] = " decimated ";
	killwords[7] = " assassinated ";
	killwords[8] = " executed ";
	killwords[9] = " murdered ";
	killword = killwords[RandomIntRange(0, 10)];
	...
	foreach ( player in level.players )
	{
		...
		if ( victim == attacker )
		{
			player iPrintLn( victim.name + " suicided" );
			continue;
		}
		if ( playerTeam == "spectator" )
		{
			player iPrintLn( "^7" + attacker.name + killword + victim.name );
			continue;
		}
		
		if ( playerTeam == victimTeam )
			player iPrintLn( "^9" + attacker.name + killword + victim.name );
		else
			player iPrintLn( "^8" + attacker.name + killword + victim.name );
